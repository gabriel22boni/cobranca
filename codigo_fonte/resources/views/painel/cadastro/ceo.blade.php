...
@section('title', 'Cadastros - CEO')

@section('content_header')
    <h1>CEO - Chief Executive Officer </h1>

    <ol class="breadcrumb">
        <li class="breadcrumb-item">Home</li>
        <li class="breadcrumb-item">Cadastros</li>
        <li class="breadcrumb-item active" aria-current="page">CEO</li>
    </ol>
@stop

@section('content')
    <div class="card card-secondary" style="position: relative; left: 0px; top: 0px;">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="col-sm-5 float-left">
                        <form action="{{ route('cadastros.ceo.search') }}" method="POST" class="form form-inline">
                            {!! csrf_field() !!}
                    ...
                    <div class="card-tools float-right">
                        @if(isset($dataForm))
                            {!! $cadCeos->appends($dataForm ?? '')->links() !!} 
                        @else
                            {!! $cadCeos->links() !!}     
                        @endif
                    </div>
                ...
                <div class="card-body p-0">
                    <table class="table">
                        ...
                        <tbody>
                            @foreach($cadCeos as $cadCeo)
                                <tr id="clienteRow{{$cadCeo->id}}">
                                    <td>{{$cadCeo->id}}.</td>
                                    <td>{{$cadCeo->nome ?? $cadCeo->fnome}}</td>
                                    <td>
                                        <center>
                                            <button onclick="modalDados({{$cadCeo->id}})" type="button" class="btn btn-primary" style="width:100px;">
                                                Ver
                                            </button>
                                        </center>
                                    </td>
                                    <td>
                                        <center>
                                            <button onclick="modalEditar({{$cadCeo->id}})" type="button" class="btn btn-secondary" style="width:100px;">
                                                Editar
                                            </button>
                                        </center>
                                    </td>
                                    <td>
                                        <center>
                                            <button onclick="modalDel({{$cadCeo->id}},'confirm')" type="button" class="btn btn-danger" style="width:100px;">
                                                Excluir
                                            </button>
                                        </center>
                                    </td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <center><p>CEO Cadastrado com sucesso.</p></center>
                </div>
...
@section('js')
    ...
    <script>
        ...
        function formSubmit (){
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('cadastros.ceo.insert') }}",
                data: {
                    _token:'{{csrf_token()}}',
                    bairro,
                    cidade,
        ...
        function pageRefresh(){
            window.location.href = "{{route('cadastros.ceo')}}";
        }

        function modalDados(id){
            $.ajax({
                url:"{{ route('cadastros.ceo.dados') }}",
                type:"post",
                datatype:"json",
                data:{
                    _token:"{{csrf_token()}}",
