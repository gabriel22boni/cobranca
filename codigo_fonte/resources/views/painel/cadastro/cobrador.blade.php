...
@section('title', 'Cadastros - Cobrador')
...
@section('content_header')
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Cobrador</li>
    </ol>
@stop
            ...
            <div class="card">
                <div class="card-header">
                    <div class="col-sm-5 float-left">
                        <form action="{{ route('cadastros.cobrador.search') }}" method="POST" class="form form-inline">
                            {!! csrf_field() !!}
                    ...
                    <div class="card-tools float-right">
                        @if(isset($dataForm))
                            {!! $cadCobs->appends($dataForm ?? '')->links() !!} 
                        @else
                            {!! $cadCobs->links() !!}     
                        @endif
                    </div>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @foreach($cadCobs as $cadCob)
                                <tr id="clienteRow{{$cadCob->id}}">
                                    <td>{{$cadCob->id}}.</td>
                                    <td>{{$cadCob->nome ?? $cadCob->fnome}}</td>
                                    <td>
                                        <center>
                                            <button onclick="modalDados({{$cadCob->id}})" type="button" class="btn btn-primary" style="width:100px;">
                                                Ver
                                            </button>
                                        </center>
                                    </td>
                                    <td>
                                        <center>
                                            <button onclick="modalEditar({{$cadCob->id}})" type="button" class="btn btn-secondary" style="width:100px;">
                                                Editar
                                            </button>
                                        </center>
                                    </td>
                                    <td>
                                        <center>
                                            <button onclick="modalDel({{$cadCob->id}},'confirm')" type="button" class="btn btn-danger" style="width:100px;">
                                                Excluir
                                            </button>
                                        </center>
                                    </td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <center><p>Cobrador Cadastrado com sucesso.</p></center>
                </div>
...
@section('js')
    ...
    <script>
        ...
        function formSubmit (){
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('cadastros.cobrador.insert') }}",
                data: {
                    _token:'{{csrf_token()}}',
                    bairro,
                    cidade,
        ...
        function pageRefresh(){
            window.location.href = "{{route('cadastros.cobrador')}}";
        }

        function modalDados(id){
            $.ajax({
                url:"{{ route('cadastros.cobrador.dados') }}",
                type:"post",
                datatype:"json",
                data:{
                    _token:"{{csrf_token()}}",
